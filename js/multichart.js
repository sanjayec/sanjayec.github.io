...
function dashboard(id, fData){
    ...
    function segColor(c){ return {"city-mpg":"#807dba", "highway-mpg":"#e08214"}[c]; }
    
    // compute total for each state.
    fData.forEach(function(d){
        d.total=(d.freq["city-mpg"]+d.freq["highway-mpg"])/2;
    });
    
    // function to handle histogram.
    function histoGram(fD){
        var hG={},    hGDim = {t: 60, r: 0, b: 60, l: 30};
        ...
        function mouseover(d){  // utility function to be called on mouseover.
            ...
            var st = fData.filter(function(s){ return s["engine-type"] == d[0];})[0],
...
//            .attr("id", "piechart_annotation")
//            .call(piechart_makeAnnotations);
    ...
    function pieChart(pD){
        ...
        function mouseover(d){
            ...
            hG.update(fData.map(function(v){ 
                return [v["engine-type"],v.freq[d.data.type]];}),segColor(d.data.type));
        }
        //Utility function to be called on mouseout a pie slice.
        function mouseout(d){
            // call the update function of histogram with all data.
            hG.update(fData.map(function(v){
                return [v["engine-type"],v.total];}), barColor);
        }
    ...
    var tF = ['city-mpg','highway-mpg'].map(function(d){ 
        return {type:d, freq: d3v3.mean(fData.map(function(t){ return t.freq[d];}))}; 
    });    
    
    // calculate total frequency by state for all segment.
    var sF = fData.map(function(d){return [d["engine-type"],d.total];});
  ...
  for(var i=1;i<lines.length;i++){
      ...
      for(var j=0;j<headers.length;j++){
          if(headers[j] === "engine-type"){
            ...
            obj.freq[key] = parseInt(currentline[j]);
        }
      }
...
fetch('data/Automobile_data_avg.csv')
...
function processCsv(text){
    ...
    dashboard('#mc_container',jsonData);
}
